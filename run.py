    ...
    for header in headers:
        ...
        if match:
            if match.startswith("static."):
                new_row.append(opportunity_owner if match == "static.Opportunity Owner" else "")
            else:
...
def get_latest_valid_word_file(word_file_dir):
    word_files = [file for pattern in ["*.doc*", "*.DOC*"]
                  for file in glob.glob(os.path.join(word_file_dir, pattern))
...
if __name__ == "__main__":
    ...
    word_file = get_latest_valid_word_file(word_file_dir)
